...
public class UthuravsUva {
    @FXML
    TextArea TEST;
    public void UthuravsUva(){
        String First_Team = "Uthura Riders";
        String Second_Team = "Uva next";

        String[] UthuraPlayers = {"pasindu", "denidu", "Thisanka", "sanga", "mahela", "sanath", "chamika", "malinga", "hasaranga", "abekoon","Theshan"};
        String[] UvaPlayers = {"Theshan", "Deshapriya", "Thiwanka", "Iresh", "anuka", "buddika", "manuka", "ginuka", "nimal", "kamal", "theminida"};


        String player = UthuraPlayers[0];
        ...
        TEST.appendText("First batman is " + UthuraPlayers[0] + "\n");
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (Ball == 0) {
                    ...
                    if (Wicket < 10) {
                        ...
                        player_position += 1;
                        player = UthuraPlayers[player_position];
        ...
        for (int b = 0; b < UthuraPlayers.length; b++) {
            Double currentWicket = Math.floor(Math.random() * 7);
            if (currentWicket < AvaWicket) {
                Wicket_ballers1.put(UthuraPlayers[b], currentWicket);
                AvaWicket -= currentWicket;
            } else {
                Wicket_ballers1.put(UthuraPlayers[b], (double) AvaWicket);
                AvaWicket = 0;
            }
        ...
        for (int b = 0; b < UvaPlayers .length; b++) {
            Double currentWicket = Math.floor(Math.random() * 7);
            if (currentWicket < AvaWicket) {
                Wicket_ballers2.put( UvaPlayers [b], currentWicket);
                AvaWicket -= currentWicket;
            } else {
                Wicket_ballers2.put( UvaPlayers[b], (double) AvaWicket);
                AvaWicket = 0;
            }
